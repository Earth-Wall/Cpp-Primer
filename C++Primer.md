...
除了【顺序容器】之外，【标准库】还支持【顺序容器的三种容器适配器】：【stack】、【queue】、【priority_queue】。
【适配器】是【标准库的通用概念】。
【容器】、【迭代器】、【函数】都有【适配器】。
【适配器的功能】和【硬件接口转换器】类似。
【容器适配器】改变了【容器的接口】。
可以向【stack】传入【除了array和forward_list之外其他的顺序容器】，【stack的接口】像【栈】。
...
| -------------------------- | -------------------------------------------------- | -------- | --------- | ---------------- |
| **构造函数**               |                                                    |          |           |                  |
|                            | 容器适配器类型<元素类型> 容器适配器变量;           |          |           |                  |
|                            | 容器适配器类型<元素类型> 容器适配器变量(容器变量); |          |           |                  |
|                            |                                                    |          |           |                  |
| **关系运算符**             |                                                    |          |           |                  |
| bool                       | < <= == != >= >                                    |          |           |                  |
|                            |                                                    |          |           |                  |
| **增加元素**               |                                                    |          |           |                  |
|                            | .push(t);                                          | 压栈     | 尾插      | 按照优先级插入   |
|                            | .emplace(t);                                       | 压栈     | 尾插      | 按照优先级插入   |
|                            |                                                    |          |           |                  |
| **访问元素**               |                                                    |          |           |                  |
| 栈顶元素的引用             |                                                    | s.top()  |           |                  |
| 首元素的引用               |                                                    |          | q.front() |                  |
| 尾元素的引用               |                                                    |          | q.back()  |                  |
| 优先级最高的元素，不是引用 |                                                    |          |           | pq.top()         |
|                            |                                                    |          |           |                  |
| **删除元素**               |                                                    |          |           |                  |
|                            | .pop();                                            | 栈顶元素 | 首元素    | 优先级最高的元素 |
|                            |                                                    |          |           |                  |
| **swap**                   |                                                    |          |           |                  |
|                            | a.swap(b);                                         |          |           |                  |
|                            | swap(a,b);                                         |          |           |                  |
...
| 元素的大小的类型           | size_type                                          |          |           |                  |
| 元素类型                   | value_type                                         |          |           |                  |
| 底层容器的类型             | container_type                                     |          |           |                  |

##### 定义一个适配器

【容器适配器】支持【两种构造函数】：
【容器适配器的默认构造函数】定义【空容器适配器变量】。
向【容器适配器的有参构造函数】传入【容器】定义【作为传入容器拷贝的容器适配器变量】。
比如，可以向【stack\<int\>的有参构造函数】传入【deque\<int\>】初始化【stack\<int\>】。
...
可以向【容器适配器的构造函数】传入【容器类型】定义【使用这种容器类型实现的容器适配器变量】。
...
只有【某些顺序容器】可以实现【容器适配器】。
因为【所有容器适配器】都需要【实现容器适配器的容器】支持添加和删除【元素】。
所以【array】不能实现【容器适配器】。
因为【所有容器适配器】都需要【实现容器适配器的容器】支持添加、删除、访问【尾元素】。
所以【forward_list】不能实现【容器适配器】。
因为【stack】需要支持【.push_back()】、【.pop_back()】、【.back()】，所以可以使用【除了array和forward_list之外其他的顺序容器】实现【stack】。
因为【queue】需要支持【.push_back()】、【.back()】、【.push_front()】、【.front()】，所以可以使用【list】或【deque】实现【queue】，不能使用【vector】实现【queue】。
因为【priority_queue】需要支持【.push_back()】、【.pop_back()】、【.front()】、【快速随机访问元素】，所以可以使用【vector】或【deque】实现【priority_queue】，不能使用【list】实现【priority_queue】。
...
参考本章的其他表格【表9.17 所有容器适配器都支持的操作和类型 P329】。

【容器适配器】改变了【容器适配器的底层容器的接口】。
只能使用【容器适配器的操作】，不能使用【容器适配器的底层容器的操作】。
...
【queue】使用【先进先出（first in first out，FIFO）的存储和访问方式】。
【queue】添加【元素】会向【queue的队尾】添加，【queue】删除【元素】会从【queue的队首】删除。
【排队】是【queue的例子】。

【priority_queue】可以设置【priority_queue的元素的优先级】。
【priority_queue添加的元素】存放在【比添加的元素的优先级低的元素】之前。
【预约】是【priority_queue的例子】。
默认情况，【标准库】使用【元素类型的小于运算符<】比较【元素的优先级】。
在【11.2.2节 P378】学习【自定义元素的比较操作】。
...
使用【泛型算法】查找、删除、替换【某个值的元素】、排列【元素】。

【泛型算法】支持【经典算法的通用接口】。
【泛型算法】支持【不同的容器类型】和【不同的元素类型】。

### 10.1 概述

【大多数的泛型算法的头文件】是【algorithm】。
...
【泛型算法】遍历【迭代器范围】，处理【迭代器范围的元素】。
比如，使用【find()】查找【vector\<int\>的某个值的元素】。
...
find()的参数。
find()的返回值。
可以比较【find()的返回值】和【查找范围的尾后迭代器】判断搜索是否成功。
