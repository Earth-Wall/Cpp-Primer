...
| insert(s1的下标,s2的参数) | insert(s1的迭代器,s2的参数) | replace(s1的下标,s1的字符的个数,s2的参数) | replace(s1的迭代器范围的首迭代器,s1的迭代器范围的尾后迭代器,s2的参数) | s2的参数可以是                              |
| ------------------------- | --------------------------- | ----------------------------------------- | ------------------------------------------------------------ | ------------------------------------------- |
| √                         |                             | √                                         | √                                                            | 字符数组                                    |
|                           |                             | √                                         | √                                                            | 字符数组,字符的个数                         |
|                           |                             | √                                         |                                                              | 字符数组,字符数组的下标,字符的个数          |
...
| 第一个新元素的迭代器                            | 容器变量A.insert(容器变量A的插入位置的迭代器,容器变量B的迭代器范围的首迭代器,容器变量B的迭代器范围的尾后迭代器); | ×     |                               |        |       |      | ×                                                            |
...
| auto/auto&接收调用.insert()的string的拷贝/本身  |                                                              |       | s1.insert(s1的位置,s2的参数)  |        |       |      |                                                              |
| auto/auto&接收调用.append()的string的拷贝/本身  |                                                              |       | s1.append(s2的参数)           |        |       |      |                                                              |
| auto/auto&接收调用.replace()的string的拷贝/本身 |                                                              |       | s1.replace(s1的位置,s2的参数) |        |       |      |                                                              |
| 新元素的迭代器                                  |                                                              |       |                               |        |       |      | fl.insert_after(插入位置之前的迭代器,元素的值);              |
| 最后一个新元素的迭代器                          |                                                              |       |                               |        |       |      | fl.insert_after(插入位置之前的迭代器,{初始化列表});          |
| 最后一个新元素的迭代器                          |                                                              |       |                               |        |       |      | flA.insert_after(flA的插入位置之前的迭代器,flB的迭代器范围的首迭代器,flB的迭代器范围的尾后迭代器); |
...
s1.replace(s1的下标, s1的字符的个数, s2的参数)。
...
s1.replace(s1的迭代器范围的首迭代器, s1的迭代器范围的尾后迭代器, s2的参数)。
...
调用【.push_back()】向【container的尾部】插入【元素】，【container.size()】增加了【1】。
【新元素的值】是【word的拷贝】。
【container的类型】可以是【list】、【vector】、【deque】。
...
虽然【某些容器】不支持【.push_front()】，但是可以调用【.insert()】向【这些容器头部】插入【元素】。

可以向【vector、deque、string的任何位置】插入【元素】。
【向vector、deque、string任何位置插入元素的速度】可能慢。

##### 插入范围内元素

可以调用【.insert()】插入【元素的个数】个【元素的值】。

```c++
容器变量.insert(插入位置的迭代器, 元素的个数, 元素的值);
```

可以调用【.insert()】插入【容器的迭代器范围的元素】，【传入的迭代器范围】不能指向【调用.insert()的容器】。

```c++
容器变量A.insert(容器变量A插入位置的迭代器, 容器变量B迭代器范围的首迭代器, 容器变量B迭代器范围的尾后迭代器);
```

可以调用【.insert()】插入【初始化列表】。
...
调用【.insert()】返回的【指向新元素的迭代器】赋值【iter】，【iter】重新指向【lst的首元素】。
重复【while循环】，读取下一个单词。
...
在调用【三个版本.emplace()】时，向【三个版本.emplace()】传入【元素类型的构造函数的参数】，【三个版本.emplace()】使用【传入的元素类型的构造函数的参数】在【调用.emplace()的容器变量的插入位置】构造【元素】。
...
可以直接调用【.front()】和【.back()】返回【首尾元素的引用】，也可以使用【\*.begin()】和【\*(--.end())】返回【首尾元素的引用】。
