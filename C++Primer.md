...
使用【.begin()】和【.end()】获取【包含容器所有元素的迭代器范围】。
...
| 容器类型<元素类型> 容器变量A=容器变量B;                      |       |                                           |        |       |      |              |
| 容器类型<元素类型> 容器变量A(容器变量B迭代器范围的首迭代器,容器变量B迭代器范围的尾后迭代器); | ×     |                                           |        |       |      |              |
| 容器类型<元素类型> 容器变量A=(容器变量B迭代器范围的首迭代器,容器变量B迭代器范围的尾后迭代器); | ×     |                                           |        |       |      |              |
...
```c++
容器类型A<元素类型A> 容器变量A(容器类型B元素类型B的变量B的子序列的首迭代器, 容器类型B元素类型B的变量B的子序列的尾后迭代器);
```
...
【左边容器的大小】等于【右边容器】。
在第二个赋值运算之后，【容器的大小】等于【初始化列表的元素的值个数】。
...
| swap(容器变量A,容器变量B);                                   |        |                   |        |       |      |              |
| 容器变量.assign({初始化列表});                               | ×      |                   |        |       |      |              |
| 容器变量A.assign(容器变量B迭代器范围的首迭代器,容器变量B迭代器范围的尾后迭代器); | ×      |                   |        |       |      |              |
| .assign(元素的个数,元素的值);                                | ×      |                   |        |       |      |              |
|                                                              |        | s1.assign(s2参数) |        |       |      |              |
...
| insert(s1下标,s2参数) | insert(s1迭代器,s2参数) | replace(s1下标,s1字符个数,s2参数) | replace(s1迭代器范围的首迭代器,s1迭代器范围的尾后迭代器,s2参数) | s2参数可以是                                |
| --------------------- | ----------------------- | --------------------------------- | ------------------------------------------------------------ | ------------------------------------------- |
| √                     |                         | √                                 | √                                                            | 字符数组                                    |
|                       |                         | √                                 | √                                                            | 字符数组,字符个数                           |
|                       |                         | √                                 |                                                              | 字符数组,字符数组下标,字符个数              |
...
在调用【.assign()】之后，【调用.assign()的容器变量】等于【传入.assign()的容器变量子序列】。
【传入.assign()的容器变量子序列】决定【调用.assign()的容器变量】的【元素的个数】和【元素的值】。

【传入.assign()的容器变量子序列】不能是【调用.assign()的容器变量的子序列】。

可以向【.assign()】传入【元素的个数】和【元素的值】。
【.assign()】拷贝【元素的个数】个【元素的值】到【调用.assign()的容器变量】。
...
在调用【swap()】之后，【指向array的迭代器、引用、指针】仍然指向【之前指向的位置】，【swap()】交换【位置上的元素的值】和【另一个容器对应位置的元素的值】。
...
【.max_size()】返回【大于等于调用.max_size()的容器变量能容纳的最大元素的个数】。
...
|                                               | .push_back(元素的值);                                        | ×     |                            |        |       |      | ×                                                            |
|                                               | .push_front(元素的值);                                       | ×     | ×                          | ×      |       |      |                                                              |
| 新元素迭代器                                  | .insert(插入位置的迭代器,元素的值);                          | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | .insert(插入位置的迭代器,{初始化列表});                      | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | 容器变量A.insert(容器变量A插入位置的迭代器,容器变量B迭代器范围的首迭代器,容器变量B迭代器范围的尾后迭代器); | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | .insert(插入位置的迭代器,元素的个数,元素的值);               | ×     |                            |        |       |      | ×                                                            |
...
| 新元素迭代器                                  |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,元素的值);              |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,{初始化列表});          |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | flA.insert_after(flA插入位置之前的迭代器,flB迭代器范围的首迭代器,flB迭代器范围的尾后迭代器); |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,元素的个数,元素的值);   |
...
s1.replace(s1迭代器范围的首迭代器, s1迭代器范围的尾后迭代器, s2参数)。
...
可以使用【.insert()】插入【元素的个数】个【元素的值】。

```c++
容器变量.insert(插入位置的迭代器, 元素的个数, 元素的值);
```

可以使用【.insert()】插入【容器的迭代器范围的元素】，【传入的迭代器范围】不能指向【调用.insert()的容器】。

```c++
容器变量A.insert(容器变量A插入位置的迭代器, 容器变量B迭代器范围的首迭代器, 容器变量B迭代器范围的尾后迭代器);
```
...
在调用【三个版本.insert()】时，向【三个版本.insert()】传入【元素的值】，【三个版本.insert()】拷贝【传入的元素的值】到【调用.insert()的容器变量的插入位置】。
...
【.end()返回的迭代器】指向【容器尾后位置】，递减【.end()返回的迭代器】，【递减之后的迭代器】指向【容器的尾元素】。
...
使用【.reserve()】通知【容器】分配【能保存这个元素的个数的缓冲区（只扩大缓冲区）】。
...
// 向【一元谓词】传入【源容器迭代器范围的元素】，剪切【源容器迭代器范围的传入一元谓词返回true的元素】到【目的容器从迭代器位置开始的元素】
目的容器粘贴范围的尾后迭代器 remove_copy_if(源容器剪切范围的首迭代器, 源容器剪切范围的尾后迭代器, 目的容器粘贴范围的首迭代器, 一元谓词);
```
...
```c++
算法名(输入范围的首迭代器, 输入范围的尾后迭代器, 其他参数);
算法名(输入范围的首迭代器, 输入范围的尾后迭代器, 输出位置的迭代器, 其他参数);
算法名(输入范围的首迭代器, 输入范围的尾后迭代器, 输出范围的首迭代器, 其他参数);
算法名(输入范围的首迭代器, 输入范围的尾后迭代器, 输出范围的首迭代器, 输出范围的尾后迭代器, 其他参数);
```
...
remove_copy_if(源容器剪切范围的首迭代器, 源容器剪切范围的尾后迭代器, 目的容器粘贴范围的首迭代器, 判断是否剪切元素的一元谓词);
```
...
使用【.lower_bound()和.upper_bound()】获取【multimap】或【multiset】中【有相同关键字的迭代器范围的迭代器范围】。
