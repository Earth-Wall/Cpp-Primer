...
| swap(容器变量A,容器变量B);                                   |        |                   |        |       |      |              |
| 容器变量.assign({初始化列表});                               | ×      |                   |        |       |      |              |
| 容器变量A.assign(容器变量B的迭代器范围的首迭代器,容器变量B的迭代器范围的尾后迭代器); | ×      |                   |        |       |      |              |
| .assign(元素的个数,元素的值);                                | ×      |                   |        |       |      |              |
|                                                              |        | s1.assign(s2参数) |        |       |      |              |
...
| insert(s1下的标,s2参数) | insert(s1的迭代器,s2参数) | replace(s1的下标,s1的字符的个数,s2参数) | replace(s1的迭代器范围的首迭代器,s1的迭代器范围的尾后迭代器,s2参数) | s2参数可以是                                |
| ----------------------- | ------------------------- | --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------- |
| √                       |                           | √                                       | √                                                            | 字符数组                                    |
|                         |                           | √                                       | √                                                            | 字符数组,字符的个数                         |
|                         |                           | √                                       |                                                              | 字符数组,字符数组的下标,字符的个数          |
...
【赋值运算符=左右两边的类型】必须相同。
【赋值运算符=】拷贝【右边容器的元素】覆盖【左边容器】。
【除array之外其他的顺序容器】支持【.assign()】，【调用.assign()的容器变量】和【传入.assign()的容器变量的子序列】的【容器类型】可以不同，【元素类型】可以不同。
【.assign()】拷贝【传入.assign()的容器变量的子序列】覆盖【调用.assign()的容器变量】。
比如，可以使用【.assign()】拷贝【vector\<char*\>】覆盖【list\<string\>】。
...
在调用【三个版本.insert()】时，向【三个版本.insert()】传入【元素的值】，【三个版本.insert()】拷贝【传入的元素的值】插入【传入.insert()的迭代器的位置】。
...
【成员函数版.merge()】剪切【源链表的输入范围的元素】插入【目的链表的输出范围】。
