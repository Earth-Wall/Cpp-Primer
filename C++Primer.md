...
【list和forward_list消耗的额外内存】比【vector、deque、array】大。
...
默认情况，【每个容器】都定义在【一个头文件】中，【容器】的【头文件名】和【类型名】相同。
...
虽然【第二个迭代器】称作【last】，但是【第二个迭代器】不指向【元素范围的尾元素】，而是指向【元素范围的尾后位置】。
【元素范围】从【第一个迭代器指向的元素】开始，到【第二个迭代器指向的位置之前的元素】结束。

【元素范围】称作【左闭合区间】，下面是标准数学描述。

```
[begin, end)
```

【元素范围】从【begin】开始，到【end之前】结束。
【begin】和【end】必须指向【相同的容器】。
【begin】和【end】可以指向【相同的位置】，【end】不能指向【begin之前的位置】。

按照顺序排列【元素范围附近的迭代器】：
元素范围的左前迭代器（元素范围之前的迭代器），
元素范围的左迭代器（元素范围的首元素的迭代器），
元素范围的右迭代器（元素范围的尾元素的迭代器），
元素范围的右后迭代器（元素范围之后的迭代器）。

【[元素范围的左迭代器, 元素范围的右后迭代器)】构成【元素范围】。

##### 使用左闭合范围蕴含的编程假定

因为【左闭合区间】的三个性质，所以使用【左闭合区间】。
假设【begin】和【end】构成【合法的元素范围】：

如果【begin】和【end】指向【相同的位置】，那么【元素范围】为空。
如果【begin】和【end】指向【不同的位置】，那么【元素范围】至少有【一个元素】，【begin】指向【元素范围的首元素】。
【begin】可以递增到等于【end】。

可以安全的使用【while循环】、【begin】、【end】处理【元素范围】。
...
传入【构成元素范围的迭代器begin和end】，如果【begin】==【end】，那么【元素范围】为空。
如果【元素范围】为空，那么直接退出【while循环】。
如果【元素范围】不为空，那么【begin】指向【元素范围的一个元素】。
因为【begin】一定指向【元素范围的一个元素】，所以在【while循环的循环体】中可以安全的解引用【begin】。
...
使用【.begin()】和【.end()】获取【包含容器所有元素的元素范围】。
...
【C++11】增加了【函数名有c的版本】，可以使用【auto】【2.5.2节 P61】接收【.begin()和.end()返回的迭代器变量】。
以前只能显式声明【迭代器的类型】。
...
在【不需要修改元素】时，应该使用【.cbegin()】和【.cend()】。
...
【除array之外其他的容器的默认构造函数】定义【空容器变量】，可以传入【容器的大小】和【元素的初始值】。
...
| 容器类型<元素类型> 容器变量A=容器变量B;                      |       |                                           |        |       |      |              |
| 容器类型<元素类型> 容器变量A(容器变量B元素范围的左迭代器,容器变量B元素范围的右后迭代器); | ×     |                                           |        |       |      |              |
| 容器类型<元素类型> 容器变量A=(容器变量B元素范围的左迭代器,容器变量B元素范围的右后迭代器); | ×     |                                           |        |       |      |              |
| 容器类型<元素类型> 容器变量(元素个数);                       | ×     | ×                                         |        |       |      |              |
| 容器类型<元素类型> 容器变量(元素个数,元素类型的值);          | ×     |                                           |        |       |      |              |
|                                                              |       | string s(字符数组);                       |        |       |      |              |
|                                                              |       | string s(字符数组,字符个数);              |        |       |      |              |
|                                                              |       | string s(字符数组,字符数组下标,字符个数); |        |       |      |              |
|                                                              |       | string s1(s2);                            |        |       |      |              |
|                                                              |       | string s1(s2,s2下标);                     |        |       |      |              |
|                                                              |       | string s1(s2,s2下标,字符个数);            |        |       |      |              |
|                                                              |       | string s1=s2.substr(s2下标);              |        |       |      |              |
|                                                              |       | string s1=s2.substr(s2下标,字符个数);     |        |       |      |              |

【string s(字符数组,整型);的整型】是【字符个数】，不是【开始下标】，和其他情况不同。
...
int main()
{
	...
	string stringC("01234", 2/*字符个数*/);               // 01
	string stringD("01234", 2/*开始下标*/, 1/*字符个数*/); // 2

	string stringE(stringA);	                         // 01234
	string stringF(stringA, 2/*开始下标*/);               // 234
	string stringG(stringA, 2/*开始下标*/, 1/*字符个数*/); // 2
...
直接拷贝【整个容器变量】，或者拷贝【除array之外其他的容器的迭代器左闭合区间的元素】。

直接拷贝【整个容器变量】，【被拷贝的容器变量】和【新定义的容器变量】的【容器类型】必须相同，【元素类型】必须相同。
拷贝【迭代器左闭合区间的元素】，【两个容器变量】的【容器类型】可以不相同，【元素类型】可以不相同。
【被拷贝容器变量的元素类型】必须可以转换为【新容器变量的元素类型】【4.11节 P141】。

```c++
容器类型A<元素类型A> 容器变量A(容器类型A元素类型A的变量B);
容器类型A<元素类型A> 容器变量A(容器类型B元素类型B的变量B.begin(), 容器类型B元素类型B的变量B.end());
```

【传入构造函数的两个迭代器】表示【拷贝范围】。
【两个迭代器】分别指向【拷贝范围的首元素】和【拷贝范围的最后一个元素之后位置】。
【新容器的大小】和【拷贝范围的元素个数】相等。
【新容器的元素】使用【拷贝范围的元素值】初始化。

可以使用【传入两个迭代器的构造函数】定义【作为另一个容器变量子序列拷贝的容器变量】。

```c++
容器类型A<元素类型A> 容器变量A(容器类型B元素类型B的变量B的子序列的首元素的迭代器, 容器类型B元素类型B的变量B的子序列的最后一个元素之后位置的迭代器);
```
...
使用【列表】初始化【容器】，显式的传入了【容器元素的初始值】。
隐含的传入了【除array之外其他的容器的大小】：
【容器的大小】和【列表的初始值个数】相等。

##### 与顺序容器大小相关的构造函数

除了【和关联容器相同的构造函数】之外，【除了array之外其他的顺序容器】支持【其他构造函数】，向【这种构造函数】传入【容器的大小】和【可以有也可以没有的元素类型的值】。

如果没有【元素类型的值】，那么标准库会创建【值初始化器】【3.3.1节 P88】。
...
如果【元素类型】是【内置类型】或【有默认构造函数的类类型】，那么【传入容器的大小和元素类型的值的构造函数】可以只传入【容器的大小】。
如果【元素类型】没有【默认构造函数】，那么【传入容器的大小和元素类型的值的构造函数】必须传入【容器的大小】和【元素类型的值】。

【关联容器】不支持【传入容器的大小和元素类型的值的构造函数】。
...
```c++
array<元素类型, 元素个数> array变量;
```

使用【array类型】必须传入【array的元素类型】和【array的大小】。

```c++
array<元素类型, 元素个数>::类型别名 变量;
```
...
【默认构造的array变量】非空：
【默认构造的array变量的元素个数】和【array变量的大小】相等。
默认初始化【默认构造的array变量的元素】【2.2.1节 P40】，和【内置数组变量的元素】【3.5.1节 P102】一样。
【列表初始化的元素类型的值个数】必须小于等于【array的大小】。
如果【列表初始化的元素类型的值个数】小于【array的大小】，那么使用【列表初始化的元素类型的值】初始化【array前面的元素】，值初始化【array剩余的元素】【3.3.1节 P88】。
并且如果【array的元素类型】是【类类型】，那么【作为元素类型的类类型】必须支持【默认构造函数】，【值初始化】需要【默认构造函数】。

```c++
array<元素类型, 元素个数> array变量 = { 初始化列表 }；
```

不能赋值或拷贝【内置数组变量】，可以赋值或拷贝【array变量】。

```
array<元素类型, 元素个数> array变量A = array变量B;
```
...
在第一个赋值运算之后，【左边容器】和【右边容器】相等。
【左边容器的大小】和【右边容器】相等。
在第二个赋值运算之后，【容器的大小】和【初始化列表的元素类型的值个数】相等。

【array】支持【赋值运算】，和【内置数组】不同。
【赋值运算符=左右两边的类型】必须相同。

```c++
array<元素类型A, 容器大小A> array变量A = { 初始化列表 };
array<元素类型A, 容器大小A> array变量B = { 0 }; // array变量B的所有元素初始化为0
array变量A = array变量B;
array变量A = { 0 }; // VS支持
```

因为【赋值运算符=左边array的大小】和【赋值运算符=右边array的大小】可能不相等，所以【array】不支持【.assign()】，不支持使用【初始化列表】赋值。
...
| swap(容器变量A,容器变量B);                                   |        |                   |        |       |      |              |
| 容器变量.assign({初始化列表});                               | ×      |                   |        |       |      |              |
| 容器变量A.assign(容器变量B元素范围的左迭代器,容器变量B元素范围的右后迭代器); | ×      |                   |        |       |      |              |
| .assign(元素个数,元素类型的值);                              | ×      |                   |        |       |      |              |
|                                                              |        | s1.assign(s2参数) |        |       |      |              |

【s1.assign(s2参数)的s2参数】和【s1.append(s2参数)的s2参数】没有限制，
【s1.insert(s1位置, s2参数)的s2参数】和【s1.replace(s1位置, s2参数)】的s2参数】必须满足下表条件。

| insert(s1下标,s2参数) | insert(s1迭代器,s2参数) | replace(s1下标,s1字符个数,s2参数) | replace(s1元素范围的左迭代器,s1元素范围的右后迭代器,s2参数) | s2参数可以是                            |
| --------------------- | ----------------------- | --------------------------------- | ----------------------------------------------------------- | --------------------------------------- |
| √                     |                         | √                                 | √                                                           | 字符数组                                |
|                       |                         | √                                 | √                                                           | 字符数组,字符个数                       |
|                       |                         | √                                 |                                                             | 字符数组,字符数组下标,字符个数          |
...
|                       |                         | √                                 | √                                                           | string                                  |
|                       |                         | √                                 |                                                             | string,string下标                       |
|                       |                         | √                                 |                                                             | string,string下标,字符个数              |
|                       | √                       |                                   | √                                                           | 元素范围的左迭代器,元素范围的右后迭代器 |
|                       |                         | √                                 | √                                                           | {初始化列表}                            |
|                       | √                       | √                                 | √                                                           | 字符个数,字符                           |
...
int main()
{
	...
	cout << stringB << endl;
	stringB.assign(sz, 2/*字符个数*/); // 01
	cout << stringB << endl;
	stringB.assign(sz, 2, 1);	   // 2
	cout << stringB << endl;
	stringB.assign(pc);			   // 01234
	cout << stringB << endl;
	stringB.assign(pc, 2/*字符个数*/); // 01
	cout << stringB << endl;
...
【赋值运算符=左右两边的类型】必须相同。
【赋值运算符=】拷贝【右边容器的元素】到【左边容器】。
【除array之外其他的顺序容器】支持【.assign()】，【调用.assign()的容器变量】和【传入.assign()的容器变量子序列】的【容器类型】可以不同，【元素类型】可以不同。
【.assign()】拷贝【传入.assign()的容器变量或容器变量子序列】到【调用.assign()的容器变量】。
比如，可以使用【.assign()】拷贝【vector\<char*\>】到【list\<string\>】。

```c++
容器变量A.assign(容器变量B元素范围的左迭代器, 容器变量B元素范围的右后迭代器);
```

在调用【.assign()】之后，【调用.assign()的容器变量】和【传入.assign()的容器变量子序列】相等。
【传入.assign()的容器变量子序列】决定【调用.assign()的容器变量】的【元素个数】和【元素类型的值】。

【传入.assign()的容器变量子序列】不能是【调用.assign()的容器变量的子序列】。

可以向【.assign()】传入【元素个数】和【元素类型的值】。
【.assign()】拷贝【元素个数】个【元素类型的值】到【调用.assign()的容器变量】。

```c++
容器变量.assign(元素个数, 元素类型的值);
```

##### 使用swap

【swap()】交换【两个容器】，【swap()交换的两个容器】的【容器类型】必须相同，【元素类型】必须相同。

```c++
swap(容器类型A元素类型A的容器变量A, 容器类型A元素类型A的容器变量B);
```

在调用【swap()】之后，容器变量A包含容器变量B的元素，容器变量B包含容器变量A的元素。
【swap()交换除array之外的容器的速度】很快，【swap()】不交换【元素】，【swap()】交换【内部数据结构】。

【swap()】不拷贝、删除、插入【除array之外容器的元素】，【swap()】是【常量阶】。

因为【swap()】不会移动【除array之外容器的元素】，所以在调用【swap()】之后，【指向除string之外容器的迭代器、引用、指针】不会失效。
【指向除string之外容器的迭代器、引用、指针】仍然指向【之前指向的元素】。
在调用【swap()】之后，【这些元素】属于【不同的容器】。
比如，在调用【swap()】之前，【迭代器】指向【容器变量A的第三个元素】，在调用【swap()】之后，【迭代器】指向【容器变量B的第三个元素】。
在调用【swap()】之后，【指向string的迭代器、引用、指针】会失效，和【其他的容器】不同。

【array】调用【swap()】交换【两个容器的元素】。
【array】调用【swap()】是【线性阶】。

在调用【swap()】之后，【指向array的迭代器、引用、指针】仍然指向【之前指向的位置】，【swap()】交换【位置上的元素值】和【另一个容器对应位置的元素值】。

【C++11】支持【成员函数版.swap()】和【非成员函数版swap()】。
【C++11之前】只支持【成员函数版.swap()】。
在【泛型编程】中需要【非成员函数版swap()】。
推荐使用【非成员函数版swap()】。

#### 9.2.6 容器大小操作

【除了forward_list其他的容器】都支持【三个和容器大小相关的成员函数】。
【.size()】返回【容器的元素个数】。
【.empty()】在【.size()返回0】时返回【true】，在【.size()返回非0】时返回【false】。
【.max_size()】返回【大于等于调用.max_size()的容器变量能容纳的最大元素个数】。
【forward_list】不支持【.size()】，原因在下一节。

参考本章其他表格【表9.9 顺序容器大小操作 P314】。

#### 9.2.7 关系运算符

【容器】都支持【相等运算符== !=】。【除了无序关联容器之外其他的容器】都支持【关系运算符< <= >= >】。
【关系运算符比较的两个容器】的【容器类型】必须相同，【元素类型】必须相同。

比较【两个容器】是逐对比较【两个容器的元素】。
【容器的关系运算符的行为】和【strcmp()】类似：

【两个容器】从【首元素】开始逐对比较。
如果【一个容器的元素】小于【另一个容器的元素】，那么【这个容器】小于【另一个容器】。
如果【一个容器的元素】一直等于【另一个容器的元素】，【两个容器的大小】不等，那么【容器大小较小的容器】小于【容器大小较大的容器】。
...
使用【容器的比较运算符】需要【元素类型】支持【比较运算符】。

【容器的相等运算符== !=】使用【元素的相等运算符== !=】，【容器的其他关系运算符】使用【元素的小于运算符<】。
使用【容器的关系运算符】需要【元素类型】支持【关系运算符】。

### 9.3 顺序容器操作

【顺序容器组织元素的方式】和【关联容器】不同。
【组织元素的方式】影响存储、访问、添加【元素】。
上一节介绍了【所有容器都支持的操作】【表9.2 容器操作 P295】。
本章剩余部分介绍【顺序容器特有的操作】。

#### 9.3.1 向顺序容器添加元素

【除array之外其他的顺序容器】都支持【灵活的内存管理】。
在运行时可以动态添加或删除【元素】，改变【容器的大小】。
【表9.5 向顺序容器添加元素的操作 P305】列出了【向除array之外其他的顺序容器添加元素的操作】。
...
|                                               | .push_back(元素类型的值);                                    | ×     |                            |        |       |      | ×                                                            |
|                                               | .push_front(元素类型的值);                                   | ×     | ×                          | ×      |       |      |                                                              |
| 新元素迭代器                                  | .insert(插入位置的迭代器,元素类型的值);                      | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | .insert(插入位置的迭代器,{初始化列表});                      | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | 容器变量A.insert(容器变量A插入位置的迭代器,容器变量B元素范围的左迭代器,容器变量B元素范围的右后迭代器); | ×     |                            |        |       |      | ×                                                            |
| 第一个新元素迭代器                            | .insert(插入位置的迭代器,元素个数,元素类型的值);             | ×     |                            |        |       |      | ×                                                            |
|                                               | .emplace_back(元素类型的构造函数参数);                       | ×     | ×                          |        |       |      | ×                                                            |
|                                               | .emplace_front(元素类型的构造函数参数);                      | ×     | ×                          | ×      |       |      |                                                              |
| 新元素迭代器                                  | .emplace(插入位置的迭代器,元素类型的构造函数参数);           | ×     |                            |        |       |      | ×                                                            |
| auto/auto&接收调用.insert()的string拷贝/本身  |                                                              |       | s1.insert(s1位置, s2参数)  |        |       |      |                                                              |
| auto/auto&接收调用.append()的string拷贝/本身  |                                                              |       | s1.append(s2参数)          |        |       |      |                                                              |
| auto/auto&接收调用.replace()的string拷贝/本身 |                                                              |       | s1.replace(s1位置, s2参数) |        |       |      |                                                              |
| 新元素迭代器                                  |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,元素类型的值);          |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,{初始化列表});          |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | flA.insert_after(flA插入位置之前的迭代器,flB元素范围的左迭代器,flB元素范围的右后迭代器); |
| 最后一个新元素迭代器                          |                                                              |       |                            |        |       |      | fl.insert_after(插入位置之前的迭代器,元素个数,元素类型的值); |
| 新元素迭代器                                  |                                                              |       |                            |        |       |      | fl.emplace_after(插入位置之前的迭代器,元素类型的构造函数参数; |
...
int main()
{
	...
	cout << stringB << endl;
	stringB.clear();
	stringB.replace(0, 0, sz, 2/*字符个数*/); // 01
	cout << stringB << endl;
	stringB.clear();
	...
	cout << stringB << endl;
	stringB.clear();
	stringB.replace(0, 0, pc, 2/*字符个数*/); // 01
	cout << stringB << endl;
	stringB.clear();
	...
	cout << stringB << endl;
	stringB.clear();
	// stringB.replace(0, 0, stringA.begin(), stringA.end()); // 01234
	// cout << stringB << endl;
	// stringB.clear();
...
s1.replace(s1元素范围的左迭代器, s1元素范围的右后迭代器, s2参数)。
...
向【vector或string尾部之外的位置】或【deque首尾之外位置】添加【元素】，需要移动【容器的元素】。
向【vector】或【string】添加【元素】，可能需要重新申请【内存】。
移动【容器的元素】到【新申请的内存】。

##### 使用push_back

使用【.push_back()】向【容器尾部】插入【元素】。

```c++
容器变量.push_back(元素类型的值);
```

【while循环】读取【string】，向【容器尾部】添加【string】。
...
传入【容器拷贝构造函数】和【容器添加元素函数】的是【参数的值拷贝】。
【容器的元素】和【传入的参数】没有关联，修改【容器的元素】不会影响【传入的参数】，修改【传入的参数】不会影响【容器的元素】，和【变量】作为【非引用参数】传入【函数】一样。

##### 使用push_front

使用【.push_front()】向【容器头部】插入【元素】。

```c++
容器变量.push_front(元素类型的值);
```

循环使用【.push_front()】向【容器头部】插入【元素序列】，【元素序列中元素传入容器的顺序】和【元素序列中元素存储在容器的顺序】相反。

##### 在容器的特定位置添加元素

使用【.insert()】向【容器任意位置】插入【元素】。
【vector】、【deque】、【list】、【forward_list】都支持【.insert()】。
【forward_list的.insert()的参数】和【其他的容器】不同，在【9.3.4节 P312】介绍。

【.insert()的第一个参数】是【迭代器】。
【迭代器】指向【向容器中插入元素的位置】。
【迭代器】可以指向【容器任何位置】，包括【容器最后一个元素之后的位置】。
【.insert()】为了可以向【容器开始位置】插入【元素】，【.insert()】向【.insert()的第一个迭代器参数指向位置】插入【元素】。

```c++
容器变量.insert(插入位置的迭代器, 元素类型的值);
```

虽然【某些容器】不支持【.push_front()】，但是可以使用【.insert()】向【这些容器头部】插入【元素】。

可以向【vector、deque、string任何位置】插入【元素】。
【向vector、deque、string任何位置插入元素的速度】可能慢。

##### 插入范围内元素

可以使用【.insert()】插入【元素个数】个【元素类型的值】。

```c++
容器变量.insert(插入位置的迭代器, 元素个数, 元素类型的值);
```

可以使用【.insert()】插入【容器的迭代器左闭合区间的元素】，【传入的迭代器左闭合区间】不能指向【调用.insert()的容器】。

```c++
容器变量A.insert(容器变量A插入位置的迭代器, 容器变量B元素范围的左迭代器, 容器变量B元素范围的右后迭代器);
```

可以使用【.insert()】插入【初始化列表】。

```c++
容器变量.insert(插入位置的迭代器, { 初始化列表 });
```

【C++11】的【.insert()】，如果【插入范围】不为空，那么返回【指向第一个新元素的迭代器】，如果【插入范围】为空，那么返回【插入位置的迭代器】。
【C++11之前】的【.insert()】返回【void】。

##### 使用insert的返回值

使用【.insert()的返回值】向【容器固定位置】插入元素。
...
初始化【iter】为【lst.begin()】，【iter】指向【lst的首元素】。
如果可以读取单词，【lst】就调用【.insert()】，向【iter位置】插入【cin读入的string】。
使用【.insert()】返回的【指向新元素的迭代器】赋值【iter】，【iter】重新指向【lst的首元素】。
重复【while循环】，读取下一个单词。
【iter】一直指向【lst的首元素】，【while循环的功能】相当于【.push_front()】。

##### 使用emplace操作

【C++11】支持【三个版本.emplace()】，【.emplace()】不拷贝【参数】构造【元素】，【.emplace()】直接使用【参数】构造【元素】。
【三个版本.emplace()】类似【三个版本.insert()】，向【容器】的【头部】、【尾部】、【任意位置】插入【元素】。

在调用【三个版本.insert()】时，向【三个版本.insert()】传入【元素类型的值】，【三个版本.insert()】拷贝【传入的元素类型的值】到【调用.insert()的容器变量的插入位置】。
在调用【三个版本.emplace()】时，向【三个版本.emplace()】传入【元素类型的构造函数参数】，【三个版本.emplace()】使用【传入的元素类型的构造函数参数】在【调用.emplace()的容器变量的插入位置】构造【元素】。
...
【.emplace_back()】和【.push_back()】都创建新变量，
【.emplace_back()】直接在【容器】创建【变量】。
【.push_back()】创建【临时变量】，向【容器】插入【临时变量的值拷贝】。

【.emplace_back()的参数】是【调用.emplace_back()的容器变量的元素类型的构造函数参数】。

#### 9.3.2 访问元素

【表9.6 在顺序容器中访问元素的操作 P305】列出了【在顺序容器中访问元素的操作】。
访问【不存在元素】，结果未定义。
【所有顺序容器】都支持【.front()】，【除forward_list之外其他的顺序容器】都支持【.back()】。
【.front()】返回【首元素引用】，【.back()】返回【尾元素引用】，在【访问元素】之前检查【元素】是否存在，【auto】不自动定义【引用类型的变量】，使用【auto&】定义【引用类型的变量】。
...
可以使用两种方法获取【容器首尾元素的引用】。
可以直接使用【.front()】和【.back()】返回【首尾元素的引用】，也可以使用【\*.begin()】和【\*(--.end())】返回【首尾元素的引用】。

【.end()返回的迭代器】指向【容器最后一个元素之后的位置】，递减【.end()返回的迭代器】，【递减之后的迭代器】指向【容器的尾元素】。
...
| 首元素引用                       | .front()    |       |                           |        |       |      |              |
| 下标n处元素引用                  | 容器变量[n] |       |                           |        |       | ×    | ×            |
| 下标n处元素引用，检查下标越界    | .at(n)      |       |                           |        |       | ×    | ×            |
| 参数第一次出现下标               |             |       | s.find(参数)              |        |       |      |              |
| 参数最后一次出现下标             |             |       | s.rfind(参数)             |        |       |      |              |
| 参数中字符第一次出现下标         |             |       | s.find_first_of(参数)     |        |       |      |              |
| 参数中字符最后一次出现下标       |             |       | s.find_last_of(参数)      |        |       |      |              |
| 参数中没有的字符第一次出现下标   |             |       | s.find_first_not_of(参数) |        |       |      |              |
| 参数中没有的字符最后一次出现下标 |             |       | s.find_last_not_of(参数)  |        |       |      |              |
...
【在容器中访问元素的成员函数】返回【元素的引用】。
【元素引用的const属性】和【容器变量的const属性】一样。
可以使用【元素的非const引用】修改【元素】。

```c++
if (!容器变量.empty())
{
	auto& 首元素引用 = 容器变量.front();
	auto& 尾元素引用 = *容器变量.begin();
}
```

【auto】不自动定义【引用类型的变量】，使用【auto&】定义【引用类型的变量】。

##### 下标操作和安全的随机访问

【支持快速随机访问的容器string、vector、deque、array】也支持【下标运算符[]】【3.3.3节 P91】。
向【下标运算符[]】传入【下标】，返回【容器下标位置元素的引用】。
【传入的下标】必须大于0，小于【容器的大小】。
【下标运算符[]】不检查【传入的下标】。
【传入的下标】越界产生错误。

可以使用【.at()】。
【.at()】检查【传入的下标】，如果【传入的下标】越界，就抛出【out_of_range异常】【5.6节 P173】。

#### 9.3.3 删除元素

【除array之外其他的顺序容器】支持【多个版本删除元素的操作】。
...
| 删除范围的右后迭代器                        | .erase(删除范围的左迭代器,删除范围的右后迭代器); | ×     |                                       |        |       |      | ×                                                         |
|                                             | .clear();                                        | ×     |                                       |        |       |      |                                                           |
| 删除位置之后的迭代器                        |                                                  |       |                                       |        |       |      | fl.erase_after(删除位置之前的迭代器)                      |
| 删除范围的右后迭代器                        |                                                  |       |                                       |        |       |      | fl.erase_after(删除范围之前的迭代器,删除范围的右后迭代器) |
| auto/auto&接收调用.erase()的string拷贝/本身 |                                                  |       | s.erase(删除开始位置的下标);          |        |       |      |                                                           |
| auto/auto&接收调用.erase()的string拷贝/本身 |                                                  |       | s.erase(删除开始位置的下标,删除个数); |        |       |      |                                                           |

删除【deque除首尾之外的元素】会使【指向deque的迭代器、引用、指针】失效。
...
使用【.pop_front()】和【.pop_back()】删除【首尾元素】。
...
使用【.erase()】删除【容器指定位置的元素】。
可以向【.erase()】传入【一个迭代器】删除【迭代器指向的元素】，也可以传入【一对迭代器】删除【迭代器左闭合区间的元素】。
...
可以向【.erase()】传入【一对迭代器】删除【迭代器左闭合区间的元素】。
返回【指向最后一个删除元素之后位置的迭代器】，下面的例子在调用【.erase()】之后，【传入.erase()】的【删除范围的左迭代器】和【删除范围的右后迭代器】相等。

```c++
删除范围的左迭代器 = 容器变量.erase(删除范围的左迭代器, 删除范围的右后迭代器);
```

可以使用【.clear()】删除【容器的所有元素】，也可以向【.erase()】传入【.begin()】和【.end()】删除【容器的所有元素】。
...
需要获取【删除或添加的元素的前驱】。
【除forward_list之外其他的容器】可以直接获取【删除或添加的元素的前驱】。
【forward_list】是【单向链表】。
...
参考本章其他表格【表9.5 向顺序容器添加元素的操作 P305】、【表9.7 顺序容器的删除操作 P311】、【9.2.3 begin和end成员的表格】。
...
| ---------------------------------------------------- | ---------------------------------- | ----------------------------------- | ----- | ------ | ------ | ----- | ---- | ------------ |
|                                                      | 已保存元素个数                     | .size()                             |       |        |        |       |      | ×            |
|                                                      | 缓冲区最大扩容后能保存元素最大个数 | .max_size()                         |       |        |        |       |      |              |
|                                                      | 容器是否为空                       | .empty()                            |       |        |        |       |      |              |
| 调整有效元素个数(不缩小缓冲区)                       |                                    | .resize(有效元素个数);              | ×     |        |        |       |      |              |
| 调整有效元素个数(不缩小缓冲区)，新元素初始化为t      |                                    | .resize(有效元素个数,元素类型的值); | ×     |        |        |       |      |              |
| 缩小capacity至size(不保证缩小)                       |                                    | .shrink_to_fit();                   | ×     |        |        |       | ×    | ×            |
|                                                      | 缓冲区目前能保存元素最大个数       | .capacity();                        | ×     |        |        | ×     | ×    | ×            |
| 通知容器分配能保存这个元素个数的缓冲区(只扩大缓冲区) |                                    | .reserve(元素个数);                 | ×     |        |        | ×     | ×    | ×            |
...
【程序】删除【vector\<int\>的奇数元素】，复制【vector\<int\>的偶数元素】。
...
使用【.reserve()】通知【容器】分配【能保存这个元素个数的缓冲区（只扩大缓冲区）】。
...
除了【3.2.1节 P76介绍的构造函数】和【其他的顺序容器的构造函数】【表9.3 容器定义和初始化 P299】之外，【string】支持【其他三个版本的构造函数】【表9.11 构造string的其他方法 P321】。

###### 表9.11 构造string的其他方法 P321

参考本章其他表格【表9.3 容器定义和初始化 P299】。
...
【string】额外支持【.append()】和【.replace()】，使用【.append()】和【.replace()】修改【string】。
...
【.replace()删除的字符个数】和【.replace()插入的字符个数】可以不相等。

###### 表9.13 修改string的操作 P323

参考本章其他表格【表9.5 向顺序容器添加元素的操作 P305】。
...
参考本章其他表格【表9.6 在顺序容器中访问元素的操作 P310】。
...
【容器适配器】改变了【容器的接口】。
向【stack】传入【除array和forward_list之外的顺序容器】，【stack的接口】像【栈】。
...
因为【stack】需要【.push_back()】、【.pop_back()】、【.back()】，所以可以使用【除array和forward_list之外其他的顺序容器】实现【stack】。
...
参考本章其他表格【表9.17 所有容器适配器都支持的操作和类型 P329】。
...
// 成功返回【第一个找到的元素的迭代器】
// 失败返回【查找范围的右后迭代器】
迭代器 find(查找范围的左迭代器, 查找范围的右后迭代器, 要查找的元素值)

头指针/迭代器 begin(内置数组/容器)
尾后指针/迭代器 end(内置数组/容器)

要查找的元素值在查找范围出现的次数 count(查找范围的左迭代器, 查找范围的右后迭代器, 要查找的元素值)

// 累加的初值 + 【迭代器左闭合区间的元素值】
累加的结果 accumulate(累加范围的左迭代器, 累加范围的右后迭代器, 累加的初值)

// 比较【第一个容器迭代器左闭合区间的元素】和【第二个容器从迭代器位置开始的元素】是否相等
// 如果两个范围大小不相等就报错
是否相等 equal(第一个容器比较范围的左迭代器, 第一个容器比较范围的右后迭代器, 第二个容器比较范围的左迭代器)

fill(赋值范围的左迭代器, 赋值范围的右后迭代器, 要赋的元素值);
fill_n(赋值范围的左迭代器, 元素个数, 要赋的元素值);

调用push_back的插入迭代器 back_inserter(容器)

// 拷贝【源容器迭代器左闭合区间的元素】到【目的容器从迭代器位置开始的元素】
目的容器粘贴范围的右后迭代器 copy(源容器拷贝范围的左迭代器, 源容器拷贝范围的右后迭代器, 目的容器粘贴范围的左迭代器)

// 搜索替换【迭代器左闭合区间的元素值】
replace(搜索替换范围的左迭代器, 搜索替换范围的右后迭代器, 要被替换的原元素值, 要替换成的新元素值);

// 搜索替换【源容器迭代器左闭合区间的元素值】，结果保存在【目的容器从迭代器位置开始的元素】
目的容器粘贴范围的右后迭代器 replace_copy(源容器拷贝范围的左迭代器, 源容器拷贝范围的右后迭代器, 目的容器粘贴范围的左迭代器, 要被替换的原元素值, 要替换成的新元素值);

// 排序【迭代器左闭合区间的元素】
sort(排序范围的左迭代器, 排序范围的右后迭代器);

// 向前提【迭代器左闭合区间的非重复元素】覆盖【重复元素】
// 不减小容器元素个数，需要手动删除多余部分
最后一个非重复元素的右后迭代器 unique(元素范围的左迭代器, 元素范围的右后迭代器);

// 向【二元谓词】传入【迭代器左闭合区间的元素】，调用【二元谓词】排序【迭代器左闭合区间的元素】
sort(排序范围的左迭代器, 排序范围的右后迭代器, 二元谓词);

// 向【二元谓词】传入【迭代器左闭合区间的元素】，调用【二元谓词】稳定排序【迭代器左闭合区间的元素】
stable_sort(排序范围的左迭代器, 排序范围的右后迭代器, 二元谓词);

// 向【一元谓词】传入【迭代器左闭合区间的元素】，调用【一元谓词】排序【迭代器左闭合区间的元素】，【返回true的元素】排在前面
partition(排序范围的左迭代器, 排序范围的右后迭代器, 一元谓词);

// 向【一元谓词】传入【迭代器左闭合区间的元素】，查找【迭代器左闭合区间的传入一元谓词返回true的元素】
// 成功返回【返回true的元素的迭代器】
// 失败返回【调用范围的右后迭代器】
迭代器 find_if(查找范围的左迭代器, 查找范围的右后迭代器, 一元谓词)

// 向【可调用对象】传入【迭代器左闭合区间的元素】，调用【可调用对象】
for_each(调用范围的左迭代器, 调用范围的右后迭代器, 可调用对象);

// 向【可调用对象】传入【源容器迭代器左闭合区间的元素】，调用【可调用对象】，返回值保存在【目的容器从迭代器位置开始的元素】
// 目的容器可以是源容器
目的容器写入范围的右后迭代器 transform(源容器调用范围的左迭代器, 源容器调用范围的右后迭代器, 目的容器写入范围的左迭代器, 可调用对象);

// 向【一元谓词】传入【迭代器左闭合区间的元素】，调用【一元谓词】
返回true的次数 count_if(调用范围的左迭代器, 调用范围的右后迭代器, 一元谓词)
...
调用insert的插入迭代器 inserter(容器, 容器插入范围的左迭代器)

// 拷贝【源容器迭代器左闭合区间的非重复元素】到【目的容器从迭代器位置开始的元素】
目的容器粘贴范围的右后迭代器 unique_copy(源容器拷贝范围的左迭代器, 源容器拷贝范围的右后迭代器, 目的容器粘贴范围的左迭代器);

正向迭代器 反向迭代器.base()

reverser(反转范围的左迭代器, 反转范围的右后迭代器)
reverser(源容器反转范围的左迭代器, 源容器反转范围的右后迭代器, 目的容器粘贴范围的左迭代器)

// 向【一元谓词】传入【迭代器左闭合区间的元素】，删除【迭代器左闭合区间的传入一元谓词返回true的元素】
第一个返回false的元素的迭代器 remove_if(删除范围的左迭代器, 删除范围的右后迭代器, 一元谓词);

// 向【一元谓词】传入【源容器迭代器左闭合区间的元素】，剪切【源容器迭代器左闭合区间的传入一元谓词返回true的元素】到【目的容器从迭代器位置开始的元素】
目的容器粘贴范围的右后迭代器 remove_copy_if(源容器剪切范围的左迭代器, 源容器剪切范围的右后迭代器, 目的容器粘贴范围的左迭代器, 一元谓词);
```
...
【泛型算法】遍历【迭代器左闭合区间】，处理【迭代器左闭合区间的元素值】。
使用【find()】查找【vector\<int\>的元素值】。

```c++
auto 迭代器变量 = find(查找范围的左迭代器, 查找范围的右后迭代器, 要查找的元素值);
if (迭代器变量 != 查找范围的右后迭代器)
{
	// 找到了要查找的元素值
}
...
find()的参数。
find()的返回值。
比较【find()的返回值】和【查找范围的右后迭代器】判断搜索是否成功。
...
```c++
auto 指针变量 = find(begin(数组变量), end(数组变量), 要查找的元素值);
if (指针变量 != end(数组变量))
{
	// 找到了要查找的元素值
}
...
【find()】可以传入【容器查找范围的迭代器左闭合区间】。

```c++
auto 指针变量 = find(数组变量 + 1, 数组变量 + 4, 要查找的元素值);
if (指针变量 != end(数组变量))
{
	// 找到了要查找的元素值
}
...
【find()除了比较元素值之外的操作】使用了【迭代器】。

##### 但算法依赖于元素类型的操作

【算法】使用了【元素类型的比较操作】。
比如，【find()】使用【元素类型的等于运算符==】比较【元素值】。
【其他算法】可能使用【元素类型的小于运算符<】比较【元素值】。
【大多数算法】可以自定义【元素比较操作】。
...
【大多数算法】操作【范围内的元素】。
【这个范围】称作【输入范围】。
【传入输入范围的算法的前两个参数】是【输入范围】，【一对迭代器】构成【迭代器左闭合区间】表示【输入范围】。
...
【从输出范围的开始位置写入元素的算法】假定【输出范围的大小】大于等于【要写入的元素个数】。

##### 介绍back_inserter

【算法】可以使用【插入迭代器】指向【输出范围的开始位置】，确保【输出范围的大小】大于等于【要写入的元素个数】。
赋值【(\*迭代器)】，赋值【迭代器指向的元素】。
赋值【(\*插入迭代器)】，向【插入迭代器指向的容器尾部】插入【和值相等的元素】。
...
```c++
fill_n(back_inserter(容器变量), 元素个数, 元素类型的值);
```
...
```c++
auto 目的数组粘贴范围的右后迭代器 copy(源数组拷贝范围的左迭代器, 源数组拷贝范围的右后迭代器, 目的数组粘贴范围的左迭代器)
```
...
【[&, =显式声明值捕获的局部变量]】表示值捕获【=号右面的局部变量】，引用捕获【其他局部变量】。
【[=, &显式声明引用捕获的局部变量]】表示引用捕获【&号右面的局部变量】，值捕获【其他局部变量】。
...
[&, =显式声明值捕获的局部变量]	         // 隐式声明和显式声明，值捕获【=号右面的局部变量】，引用捕获【lambda表达式的函数体使用的其他局部变量】
[=, &显式声明引用捕获的局部变量]       // 隐式声明和显式声明，引用捕获【=号右面的局部变量】，引用捕获【lambda表达式的函数体使用的其他局部变量】
```
...
保持【ostream_iterator的使用方法】和【其他迭代器】一致。
方便把【ostream_iterator】修改为【其他迭代器类型】。
增加【程序可读性】。
...
【除forward_list之外其他的容器】都支持【递增运算符++】和【递减运算符--】。
...
```c++
算法名(输入范围的左迭代器, 输入范围的右后迭代器, 其他参数);
算法名(输入范围的左迭代器, 输入范围的右后迭代器, 输出位置的迭代器, 其他参数);
算法名(输入范围的左迭代器, 输入范围的右后迭代器, 输出范围的左迭代器, 其他参数);
算法名(输入范围的左迭代器, 输入范围的右后迭代器, 输出范围的左迭代器, 输出范围的右后迭代器, 其他参数);
```
...
```c++
unique(元素范围的左迭代器, 元素范围的右后迭代器);
unique(元素范围的左迭代器, 元素范围的右后迭代器, 比较元素的二元谓词);
```

【两个版本的unique()】都向前提【元素范围的非重复元素】。
因为【两个版本的unique()的参数个数】不等，所以调用【unique()】不会出现【重载歧义】。

##### _if版本的算法

【传入元素值参数的算法】有【另一个版本的传入一元谓词参数替代元素值参数的算法】，【两个版本的算法的名称】不同。
【传入一元谓词参数替代元素值参数的算法的名称】有【_if后缀】。

```c++
find(查找范围的左迭代器, 查找范围的右后迭代器, 要查找的元素值);
find_if(查找范围的左迭代器, 查找范围的右后迭代器, 查找元素的二元谓词);
```

【两个版本的find()】都在【输入范围】查找【特定元素第一次出现的位置】。
【find()】返回【范围内第一个等于元素值参数的元素的迭代器】。
...
【一些算法】有【同时有\_copy和\_if的版本】，【同时有\_copy和\_if的算法版本】有【输出范围的左迭代器参数】和【一元谓词参数】。

```c++
remove_if(删除范围的左迭代器, 删除范围的右后迭代器, 判断是否删除元素的一元谓词);
remove_copy_if(源容器剪切范围的左迭代器, 源容器剪切范围的右后迭代器, 目的容器粘贴范围的左迭代器, 判断是否剪切元素的一元谓词);
```

【判断是否剪切元素的一元谓词】可以是【lambda表达式】。

### 10.6 特定容器算法

【list】和【forward_list】支持【成员函数版算法】，和【其他的容器】不同。
...
【通用版算法】需要交换【输入范围的元素值】。
【链表】可以通过改变【节点顺序】实现交换【元素值】。
【链表的成员函数版算法的性能】更好。

###### 表10.6 list和forward_list成员函数版本的算法 P369

```c++
// 合并【listB的元素】到【listA】，排序【listA的元素】，【listB】变为空
listA.merge(listB)

// 合并【listB的元素】到【listA】，调用【二元谓词】排序【listA的元素】，【listB】变为空
...
```c++
// 剪切【listB的所有元素】插入到【listA的迭代器位置】，【listB】变为空
// 【listA】和【listB】不能相同
listA.splice(listA迭代器, listB);

// 剪切【forward_listB的所有元素】插入到【forward_listA的迭代器位置之后】，【forward_listB】变为空
// 【forward_listA】和【forward_listB】不能相同
forward_listA.splice_after(forward_listA迭代器, forward_listB);

// 剪切【listB的迭代器指向的元素】插入到【listA的迭代器位置】
// 【listA】和【listB】可以相同
listA.splice(listA迭代器, listB, listB迭代器);

// 剪切【forward_listB的迭代器指向的元素】插入到【forward_listA的迭代器位置之后】
// 【forward_listA】和【forward_listB】可以相同
forward_listA.splice_after(forward_listA迭代器, forward_listB, forward_listB迭代器);

// 剪切【listB迭代器左闭合区间的元素】插入到【listA的迭代器位置】
// 【listA】和【listB】可以相同，【listA迭代器】不能在【listB迭代器左闭合区间】
listA.splice(listA迭代器, listB, listB左迭代器, listB右后迭代器);

// 剪切【forward_listB迭代器左闭合区间的元素】插入到【forward_listA的迭代器位置之后】
// 【forward_listA】和【forward_listB】可以相同，【forward_listA迭代器】不能在【forward_listB迭代器左闭合区间】
forward_listA.splice_after(forward_listA迭代器, forward_listB, forward_listB左迭代器, forward_listB右后迭代器);
```

##### 链表特有的操作会改变容器

【链表的成员函数版算法】和【通用版算法】类似，但是不完全相同。
【链表的成员函数版算法】会改变【底层容器】。
比如，【成员函数版.remove()】会删除【指定元素】。
【成员函数版.unique()】会删除【重复元素】。

【成员函数版.merge()】和【成员函数版.splice()】会销毁【输入范围的元素】。
【通用版merge()】复制【两个输入范围的元素】到【输出范围】。
在调用【通用版merge()】之后，【两个输入范围的元素】不变。
【成员函数版.merge()】剪切【源链表的输入范围的元素】到【目的链表的输出范围】。
在调用【成员函数版merge()】之后，【源链表的两个输入范围的元素】仍然存在，但是【元素】不在【源链表的两个输入范围】，在【目的链表的输出范围】。
...
【关联容器】不支持【有【一个元素值参数】和【一个元素个数参数】的【构造函数】或【插入函数】】。
...
使用【.begin()和.end()】获取【迭代器】，使用【迭代器】遍历容器。
...
关联容器变量.insert(插入value_type类型的值的范围的左迭代器, 插入value_type类型的值的范围的右后迭代器)
...
删除位置之后的迭代器 关联容器变量.erase(删除范围的左迭代器, 删除范围的右后迭代器);
```
...
使用【.find()】和【.count()】查找【非multi类型关联容器的效果】相同。
使用【.count()】查找【multi类型关联容器】会统计【关键字出现次数】。
...
只想要在【map】中查找【元素】，不想要插入【元素】，应该使用【.find()】。
...
使用【.find()】查找【这个作者的第一个作品】。
使用【.count()】获取【这个作者的作品个数】。

从【这个作者的第一个作品】开始循环打印。

##### 一种不同的，面向迭代器的解决办法

使用【.lower_bound()和.upper_bound()】获取【multimap】或【multiset】中【有相同关键字的元素范围的迭代器左闭合区间】。
...
使用【.lower_bound()和.upper_bound()】打印在【multimap】或【multiset】中存储的【一个作者的所有作品】。

##### euqal_range函数

使用【.equal_range()】打印在【multimap】或【multiset】中存储的【一个作者的所有作品】。
...
size_t桶的元素个数 无序容器变量.bucket_size(0)
size_t关键字所在桶 无序容器变量.bucket(0)
float每个桶的平均元素个数 无序容器变量.load_factor()
float容器想要的每个桶的平均元素个数 无序容器变量.max_load_factor()

无序容器变量.rehash(桶个数);
无序容器变量.reserve(保存元素个数);
...
int main()
{
	...
	size_t aC = unordered_mapA.bucket_size(0);	   // 0，桶的元素个数
	size_t aD = unordered_mapA.bucket(0);		   // 5，关键字所在桶
	float aE = unordered_mapA.load_factor();	   // 0.625，每个桶的平均元素个数
	float aF = unordered_mapA.max_load_factor();   // 1，容器想要的每个桶的平均元素个数

	unordered_mapA.rehash(10);	// 设置桶个数
	unordered_mapA.reserve(10);	// 设置容器可以保存元素个数，并且不需要rehash？？？
...
```c++
shared_ptr变量 make_shared<shared_ptr变量指向类型>(shared_ptr变量指向类型的构造函数参数)
...
必须初始化【const动态变量】。
可以隐式初始化【有默认构造函数的const动态类对象】，其他类型必须显式初始化。
...
可能有多个【内置指针】指向【相同的动态内存】，delete【这块动态内存】之后，【一个指向这块动态内存的内置指针】=nullptr，【其他指向这块动态内存的内置指针】还指向【这块动态内存】。
...
如果使用【自定义释放操作】替代【delete】，那么【智能指针】可以指向【其他类型】。
...
使用【.get()】获取【内置指针】，【这个内置指针】传入【不能使用智能指针的函数】，【这个函数】不能释放【内置指针】。

不能使用【.get()获取的内置指针】初始化【智能指针】，否则会产生【两个独立的智能指针】指向【相同的动态变量】。
【一个智能指针】销毁【动态变量】时，【另一个智能指针】还会使用【这个动态变量】，产生错误。
销毁【第二个智能指针】时会第二次释放【这块动态内存】。

##### 其他shared_ptr操作

可以使用【.reset()】传入【内置指针】，【智能指针】会指向【内置指针指向的动态变量】。
【.reset()】会更新【引用计数】。
使用【.reset()】和【.unique()】管理【多个shared_ptr指向的动态变量】，【一个shared_ptr】修改【这个shared_ptr指向的动态变量】之前，先调用【.unique()】检查【这个shared_ptr指向的动态变量】是否只被【自己】指向。
如果只有【自己】指向【这个动态变量】，那么可以直接修改【这个动态变量】。
如果还有【其他智能指针】指向【这个动态变量】，那么调用【.reset()】传入【【这个动态变量】初始化的【new】】，使得【自己】指向【一个新的动态变量】，修改【这个新的动态变量】。
...
不使用【.get()返回的内置指针】初始化或.reset()【智能指针】。
...
使用【.release()】或【.reset()】让【另一个指针】指向【有指针指向的资源】。
...
大多数应用不需要直接访问【动态数组变量】。
应用需要【可变数量的变量】时，应该使用更简单、更快速、更安全的【vector或其他标准库的容器】，比如【StrBlob类】。
...
```c++
// 【动态数组的元素个数】必须是整型，可以是常量或非常量
动态数组的元素类型* 指向动态数组开头的指针变量 = new 动态数组的元素类型[动态数组的元素个数];



// 定义【指定元素类型和元素个数的数组类型】
typedef 数组的元素类型 数组类型[数组的元素个数];
...
可以在【new表达式】最后写上【一对花括号包围的元素类型的值】，列表初始化【使用new申请的动态数组】。

【new表达式】和【内置数组】的【列表初始化】的行为类似。
如果【元素类型的值个数】大于【元素个数】，那么【new表达式】失败，不申请内存。
会抛【出bad_array_new_length类型异常】。
【bad_array_new_length类型的头文件】是【new】。
使用【一对空圆括号】值初始化【使用new申请的动态数组】，不能在括号中写元素类型的值，不能在【new表达式】中使用【auto】申请【动态数组】。
...
使用【new】申请【元素个数0的动态数组】，【new】返回【合法非空指针】。
【这个指针】和【new返回的其他指针】不同。
...
【shared_ptr】不支持【下标运算符】和【算术运算符】，可以使用【.get()】获取【指向shared_ptr指向的动态变量的内置指针】，使用【这个内置指针】访问【动态数组的元素】。
...
// 构造动态变量
allocator变量.construct(动态变量类型的指针, 动态变量的构造函数参数);
...
需要在【动态内存】构造【动态变量】。
【成员函数.construct()】传入【一个指针】和【动态变量的构造函数参数】，在【指针地址】构造【动态变量】。
使用【传入的动态变量的构造函数参数】构造【动态变量】。
【.construct()】和【make_shared()】的参数类似，【传入的动态变量的构造函数参数】匹配【动态变量的构造函数】。

不能使用【没有构造动态变量的动态内存】。

必须使用【.destory()】销毁【用完的动态变量】。
【.destory()】传入【一个指针】，析构【指针指向的动态变量】。

使用【while循环】和【.destory()】从后往前销毁【.construct()构造的动态变量】。

可以重新使用【销毁的动态变量所在的动态内存】，也可以释放【销毁的动态变量所在的动态内存】。
使用【.deallocate()】释放【销毁了动态变量的动态内存】。
...
```c++
uninitialized_copy(拷贝范围的左迭代器, 拷贝范围之后的迭代器, 目的动态内存的迭代器);
uninitialized_copy_n(拷贝开始位置的迭代器, 拷贝个数, 目的动态内存的迭代器);
uninitialized_fill(动态内存范围的左迭代器, 动态内存范围之后的迭代器, 动态变量初始值);
uninitialized_fill_n(动态内存开始位置的迭代器, 创建动态变量个数, 动态变量初始值);
```

使用【.allocate()】申请【未初始化的动态内存】。
...
【前两个迭代器参数】构成【拷贝范围的迭代器左闭合区间】，【第三个迭代器参数】指向【目的动态内存】。
...
如果一个单词在一行出现多次，那么多次调用.insert()，第一次调用之后的其他调用什么也不做。
