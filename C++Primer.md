...
除了【固定大小的array】之外，【其他顺序容器】都可以高效灵活的管理【内存】。
【其他顺序容器】可以添加和删除【容器的元素】，扩大和缩小【容器的大小】。
...
【除了array之外其他容器的默认构造函数】定义【空容器变量】，可以传入【容器的大小】和【元素的初始值】。
...
除了【和关联容器相同的构造函数】之外，【除了array之外其他顺序容器】还支持【其他构造函数】，向【这种构造函数】传入【容器的大小】和【可选的元素的初始值】。
...
【赋值运算符=左右两边的类型】必须相同。
【赋值运算符=】拷贝【右边容器的元素】覆盖【左边容器】。
【除了array之外其他顺序容器】支持【.assign()】，【调用.assign()的容器变量】和【传入.assign()的容器变量的子序列】的【容器类型】可以不同，【元素类型】可以不同。
...
【swap()交换除了array之外其他容器的速度】很快，【swap()】不交换【元素】，【swap()】交换【容器的内部数据结构】。

【swap()】不拷贝、删除、插入【除了array之外其他容器的元素】，【swap()】交换【两个除了array之外其他容器的元素】是【常量阶】。

因为【swap()】不会移动【除了array之外其他容器的元素】，所以在调用【swap()】之后，【除了string之外其他容器的迭代器、引用、指针】不会失效。
虽然【除了string之外其他容器的迭代器、引用、指针】仍然指向【之前指向的元素】。
但是在调用【swap()】之后，【这些元素】属于【不同的容器】。
比如，在调用【swap()】之前，【迭代器】指向【容器变量A的第三个元素】，在调用【swap()】之后，【迭代器】指向【容器变量B的第三个元素】。
在调用【swap()】之后，【string的迭代器、引用、指针】会失效，和【其他容器】不同。

【swap()】交换【两个array的元素】，和【其他容器】不同。
...
【除了forward_list之外其他容器】都支持【三个和容器大小相关的成员函数】。
...
【除了array之外其他顺序容器】都支持【灵活的内存管理】。
在运行时可以动态添加或删除【元素】，改变【容器的大小】。
【表9.5 向顺序容器添加元素的操作 P305】列出了【向除了array之外其他顺序容器添加元素的操作】。
...
【除了array和forward_list之外其他顺序容器】支持【.push_back()】。
...
【forward_list的.insert()的参数】和【其他容器】不同，在【9.3.4节 P312】介绍。
...
【所有顺序容器】都支持【.front()】，【除了forward_list之外其他顺序容器】都支持【.back()】。
【.front()】返回【首元素引用】，【.back()】返回【尾元素引用】。
在【访问元素】之前检查【元素】是否存在，【auto】不自动定义【引用类型的变量】，使用【auto&】定义【引用类型的变量】。
...
【除了array之外其他顺序容器】支持【多个版本删除元素的操作】。
...
【forward_list的添加和删除函数】和【其他容器】不同的原因。

【容器】添加或删除【元素】，需要修改【删除或添加的元素的前驱的后继】。
需要获取【删除或添加的元素的前驱】。
【除了forward_list之外其他容器】可以直接获取【删除或添加的元素的前驱】。
【forward_list】是【单向链表】。
...
因为【forward_list的添加和删除函数】和【其他容器】不同，所以【forward_list的添加和删除函数名称】是【.insert_after()】、【.emplace_after()】、【.erase_after()】【表9.8 在forward_list中插入和删除元素的操作 P313】。
...
除了【3.2.1节 P76介绍的构造函数】和【其他顺序容器的构造函数】【表9.3 容器定义和初始化 P299】之外，【string】支持【其他三个版本的构造函数】【表9.11 构造string的其他方法 P321】。
...
因为【stack】需要【.push_back()】、【.pop_back()】、【.back()】，所以可以使用【除了array和forward_list之外其他顺序容器】实现【stack】。
...
【反向迭代器】：【反向迭代器】绑定【容器变量】，递增【反向迭代器】，【反向迭代器】向【容器头部】移动。【除了forward_list其他容器】都支持【反向迭代器】。
...
【除了forward_list之外其他容器】都支持【递增运算符++】和【递减运算符--】。
...
【list】和【forward_list】支持【成员函数版本的算法】，和【其他容器】不同。
